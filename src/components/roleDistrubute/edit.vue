<template>
  <li :class="[isButton, hasBorder]" style="list-style:none;">
    ...
    <ul v-show="open" v-if="isFolder">
      <tree-menu-edit v-for="(item, index) in model.childNode" :model="item" :key="index" :menuList="menuList" :label="label" :selectKeys="selectKeys" @getkeys="getkeyArr"></tree-menu-edit>
    </ul>
  </li>
</template>
    ...
    mounted () {
      ...
      if (this.selectKeys instanceof Array && this.selectKeys.length > 0 && this.selectKeys.indexOf(this.model.id) >= 0) {
        if (this.model.childNode && this.model.childNode.length && this.model.menuLevel !== 1) { // 包含子级,一级菜单除外
          // 计算所有子节点是否全部选中
            $('#menu' + this.model.id).first().prop("checked", true);
        } else {
          $('#menu' + this.model.id).first().prop("checked", true);
        }
      }
