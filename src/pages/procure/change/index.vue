                ...
                <el-tab-pane label="收货数据" name="first">
                    ...
                    <table v-if="inventories.length>0" v-for="(item,index) in inventories" border="1" bordercolor="#E5E5E5" style="width:100%;text-align: center;">
                        ...
                        <tr >
                            <td>散装</td>
                            <td colspan="3">/</td>
                            <td colspan="3">{{item.remained_stock}}{{getEnumValue(enums.units,item.base_unit_id)}}</td>
                            <td colspan="1"> <el-input v-model="item.base_quantity"  size="small" @input="change_base_quantity(item,2)" style="display: inline-block; width: 100px; border: 1px solid #f2f2f2;margin-right: 5px;"></el-input>{{getEnumValue(enums.units,item.base_unit_id)}} </td>
                        ...
                        <tr v-for="(i,ind) in item.packeds">
                                <td>包装</td>
                                <td colspan="3">{{i.packsize}}{{getEnumValue(enums.units,i.unit_id)}}</td>
                                <td colspan="3">{{i.stock}}
                                    ({{(Number(i.stock)*Number(i.packsize))}}{{getEnumValue(enums.units,i.unit_id)}})
                                </td>
                                <td colspan="1"> 
                                    <el-input  size="small" v-model="i.base_quantity" @input="change_base_quantity(i,1)"  style="display: inline-block; width: 100px; border: 1px solid #f2f2f2;margin-right: 5px;"></el-input>
                                    个
                                </td>
                                <td>
                                    <el-checkbox :disabled="i.stock==0?true:false"  v-model="i.checked" @change="check_inventories_pack(i.checked,item,i)" :true-label="1" :false-label="0"></el-checkbox>
                                </td>
                            </tr>
                    </table>
                    <el-form  v-if="inventories.length>0" :inline="true" class="demo-form-inline">
                        ...
                        <el-form-item label="退货备注" style="width:45%;">
                            ...
                            <!--<el-select size="mini" v-model="refund_inventories.return_to"  readonly style="width: 150px;margin-right: 10px;color: red;">-->
                                <!--<el-option-->
                                        <!--label="退给供应商"-->
                                        <!--value="退给供应商"-->
                                <!--&gt;</el-option>-->
                                ...
                                <!--&gt;</el-option>-->
                            <!--</el-select>-->
                            <span class="red bold" style="margin-right: 10px;">{{refund_inventories.return_to}}</span>
                ...
                <el-tab-pane label="付款数据" name="second">
                    <table v-if="paylists.length>0" border="1" bordercolor="#E5E5E5" style="width:100%;">
                        ...
                        <tr >
                            ...
                            <td><el-checkbox  v-if="index==0" v-model="refund_credit_flag" :true-label="1" :false-label="0">往来账</el-checkbox>
                            </td>
                            <td colspan="7">
                                <span v-if="!refund_credit_flag">
                                        收款银行
                                        <el-select
                                                v-model="paybank"
                                                filterable
                                                clearable
                                                size="mini"
                                                :disabled="paybank_type=='company_bank'?true:false"
                                                placeholder="收款公司"
                                                class="select"
                                                style="width: 300px;"
                                        >
                                            <el-option v-for="(i,index) in enums.paybanks" :key="index" :label="i.bank" :value="i.bank"></el-option>
                                        </el-select>
                                    </span>
                                <el-button v-if="index==0" style="margin-left: 10px;" type="primary" size="mini" @click="save_pay(paylists[0].id,refund_amount_pay,refund_credit_flag)">保存</el-button>
                            </td>
                        </tr>
                    </table>
                </el-tab-pane>
...
<script>
    ...
    export default {
        ...
        methods: {
            change_base_quantity(item,type){
                this.refund_inventories.returns.forEach((items, index) => {
                    if(type==2){
                        if(items.type==type&&items.inventory_id == item.id) {
                            items.base_quantity=item.base_quantity
                        }
                    }else if(type==1){
                        if(items.type==type&&items.package_id == item.id) {
                            items.package_nums=item.base_quantity
                            items.base_quantity = Number(item.base_quantity)*Number(item.packsize)
                        }
                    }
                })
            },
            ...
            check_inventories(checked,item){
                if (checked) {
                    this.refund_inventories.returns.push({
                        ...
                        base_quantity_unit:item.base_unit_id,
                        package_id:0,
                        package_nums:1,
                    ...
                    this.refund_inventories.returns.forEach((items, index) => {
                        if (items.type==2&&items.inventory_id == item.id) {
                            this.refund_inventories.returns.splice(index, 1)
                        }
                    })
                }
            },
            check_inventories_pack(checked,item,i){
                if (checked) {
                    this.refund_inventories.returns.push({
                        type:1,
                        prod_id:item.prod_id,
                        batchno:item.batchno,
                        contract_detail_no:item.purchaseorder_no,
                        base_quantity:(Number(i.base_quantity)*Number(i.packsize)),
                        base_quantity_unit:i.unit_id,
                        package_id:i.id,
                        package_nums:i.base_quantity,
                        packsize:i.packsize,
                    ...
                    this.refund_inventories.returns.forEach((items, index) => {
                        if (items.type==1&&items.package_id == i.id) {
                            this.refund_inventories.returns.splice(index, 1)
                        }
                    })
                }
            ...
            save_pay(id,refund_amount,refund_credit_flag){
                ...
                this.refund_pay.corp_name=this.sale.corp_name
                if(!refund_credit_flag){
                    this.refund_pay.paybank=this.paybank
                }else{
                    this.refund_pay.paybank=''
                }
            ...
            fetchData() {
                ...
                ProcurementContract(this.$route.query.ids).then(res => {
                    res.data.contract.details.forEach((item,index)=>{
                        ...
                        if(item.is_from_consign){
                            this.refund_inventories.return_to='退到寄售库'
                        }else{
                            this.refund_inventories.return_to='退给供应商'
                        }
                    })
                    this.sale = res.data.contract
                    res.data.inventories.forEach((item,index)=>{
                        item.checked=false
                        item.base_quantity=item.remained_stock
                        if(item.packeds&&item.packeds.length>0){
                            item.packeds.forEach(i=>{
                                i.checked=false
                                i.base_quantity=i.stock
                            })
                        }
                    })
                    ...
                    this.paybank_type=res.data.paybank_type
                    this.enums = res.enum
                    this.receive_invoice_addresses=[]
                    this.receive_addresses=[]
