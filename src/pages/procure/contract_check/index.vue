  ...
  <d2-container>
    ...
    <div class="heard">
      ...
      <el-select
              ...
              clearable
      >
        <el-option v-for="item in supply_list"  :key="item.id" :label="item.name" :value="item.id"></el-option>
      </el-select>
      <el-date-picker
              class="date_picker"
    ...
    <div style="width:100%;" id="id">
      <el-table
        ...
        <!--</template>-->
        <!--</el-table-column>-->
        <el-table-column align="center" label='合同号' width="125">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" label='付款状态'>
          <template slot-scope="scope">
            <div v-if="!scope.row.procurement_pay_result && scope.row.paylistexist" style="background-color: rgb(255, 193, 7);">
              <span>已申请</span>
              <span v-if="scope.row.offset_amount" style="color: red;">(往)</span>
            </div>
            <div v-else :style="{backgroundColor:scope.row.backgroundColor_pay}">
              <span v-if="scope.row.procurement_pay_result">
                <span
                        v-if="Number(scope.row.procurement_pay_result.pay_requestamount)<=0"
                >已申请
                </span>
                <span
                        style="display: inline-block;width: 100%;"
                        v-if="Number(scope.row.procurement_pay_result.pay_requestamount)>0 && Number(scope.row.procurement_pay_result.pay_requestamount)<Number(scope.row.total_money)"
                >部分付款</span>
                <span
                        style="display: inline-block;width: 100%;"
                        class
                        v-if="Number(scope.row.procurement_pay_result.pay_requestamount)==Number(scope.row.total_money)"
                >全额付款</span>
                <span
                        style="display: inline-block;width: 100%;"
                        v-if="Number(scope.row.procurement_pay_result.pay_requestamount)>Number(scope.row.total_money)"
              ...
              <span v-else>
                <span  v-if="!scope.row.paylistexist">未申请</span>
              </span>
              <span v-if="scope.row.offset_amount" style="color: red;">(往)</span>
            </div>
          </template>
        ...
        <el-table-column align="center" label='收货状态'>
          <template slot-scope="scope">
            ...
            <!--<div slot="content">-->
            <!--<span v-for="detail in scope.row.details">-->
            <!--<span v-text="detail.storage_status==2?'部分入库':(detail.storage_status==1?'已入库':(detail.storage_status==4?'超额入库':(detail.storage_status==0?'未入库':'')))"></span>;-->
            <!--</span>-->
            <!--</div>-->
            <!--<div v-if="scope.row.details.length>0">....-->
            <!--&lt;!&ndash;<span v-text="scope.row.details[0].storage_status==2?'部分入库':(scope.row.details[0].storage_status==1?'已入库':(scope.row.details[0].storage_status==4?'超额入库':(scope.row.details[0].storage_status==0?'未入库':'')))"></span>...&ndash;&gt;-->
            <!--</div>-->
            <!--</el-tooltip>-->
          </template>
        </el-table-column>
        <el-table-column align="center" label='发票状态'>
          <template slot-scope="scope">
            <div :style="{backgroundColor:(scope.row.invoice_type=='不含税'?'#C0C0C0':(scope.row.invoice_type=='全额收票'?'#FFC0CB':(scope.row.invoice_type=='部分收票'?'#FFF0DD':(scope.row.invoice_type=='超额收票'?'#F5DEB3':''))))}">{{scope.row.invoice_type}}</div>
            <!--<el-tooltip placement="top">-->
            <!--<div slot="content">-->
            <!--<span v-for="invoice_type in scope.row.invoice_types">-->
            <!--{{invoice_type}};-->
            <!--</span>-->
            <!--</div>-->
            <!--<div v-if="scope.row.invoice_types.length>0">-->
            <!--{{scope.row.invoice_types[0]}}...-->
            <!--</div>-->
            <!--</el-tooltip>-->
          </template>
        </el-table-column>
        <el-table-column
                ...
                width="200">
          <template slot-scope="scope">
            <div v-if="permissions.indexOf(89)>-1 && scope.row.is_old!=1" style="display: inline-block;position: absolute;left: -5px;">
              <el-button type="text" @click="delivery(scope.row)" size="mini">
                <i class="el-icon-truck" style="color: red;" v-if="scope.row.express_numbers&&scope.row.express_numbers.length>0"></i>
              </el-button>
            </div>
...
<script>
    ...
    import {cancel_contract} from "@/api/procure";
    import {
        search_menu
    } from "@/api/Inquery"
