    ...
    <d2-container>
        <div>
            <el-row>
                <el-col :span="24" style="text-align: right;">
                    ...
                    <el-select
                            size="mini"
                            v-model="queryList.verify_user_id"
                            filterable
                            placeholder="选择审核人员"
                            style="width:130px;"
                            clearable
        ...
        <el-table :data="list.data" @selection-change="handleSelectionChange" border fit highlight-current-row class="d2-mt-10">
            ...
            <el-table-column align="center" label="审核人员">
                <template slot-scope="scope">
                <span>
                    {{scope.row.contract_verify_assignment?scope.row.contract_verify_assignment.verify_user.name:''}}
                </span>
                </template>
            </el-table-column>
            <el-table-column align="center" width="100">
                <template slot="header">
                    <el-button  
                        type="primary"  
                        size="mini"
                        v-if="permission_names['saler_contract_set.contract_review_allocation']=='saler_contract_set.contract_review_allocation'"
        ...
        <el-dialog title="审核分配" :visible.sync="dialogDue" class="form" width="40%">
            <el-form label-width="100px" label-position="right">
                <el-form-item label="选择的人员:">
                    <!-- <span v-for="(i,index) in user_ids_list">
                        <span>{{i.name}}</span>
                        <span v-if="index!=(user_ids_list.length-1)">,</span>
                    </span> -->
                    <el-row>
                ...
                <el-form-item label="审核人:">
                    <el-select
                            size="small"
                            v-model="due_data.verify_user_id"
                            filterable
                            popper-class='select_height'
                            clearable
                    >
                        <el-option v-for="item in UsersList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                    </el-select>
...
export default {
  ...
  watch: {
    $route (to, from) {
        if(to.name=='sales-audit_allocation'){
        }
    }
  },
  created () {
  ...
  methods: {
    fetchData () {
      contract_review_allocation_list(this.queryList).then(res => {
    ...
    check(item){
        ...
        this.due_data={
            verify_user_id:item.contract_verify_assignment?item.contract_verify_assignment.verify_user_id:'',
            user_ids:[]
        }
        this.dialogDue = true
    },
    ...
    save_due(){
        ...
        contract_review_allocation(this.due_data).then(res=>{
            if (res && res.code == 0) {
                this.$message({
                    message: 'success',
                    type: 'success'
