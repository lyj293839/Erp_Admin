          ...
          <el-form-item v-show="form_show2" class="from-item">
            <table border="1" bordercolor="#E5E5E5">
              ...
              <tr v-for="item in detail.details">
                ...
                <td>{{item.cas}}</td>
                <td>
                    <span>{{item.saler_contract_detail.product_name_cn?item.saler_contract_detail.product_name_cn:item.product_name}}</span>
                    <el-tooltip placement="top" v-if="item.is_danger_inland">
                        <div slot="content">
                            <span>{{getEnumValue(danger_inland_status,item.is_danger_inland)}}</span>
                        </div>
          ...
          <el-form-item v-show="form_show4" class="from-item" label>
            <div class="d2-m-10">
              ...
              <el-radio-group v-model="form.status">
                <el-radio :label="-9">{{$t('sale.add.sale87')}}</el-radio>
                <!-- 审合通过 -->
                <el-radio :label="-8">{{$t('sale.add.sale88')}}</el-radio>
                <!-- 不通过 -->
              </el-radio-group>
            </div>
            <div class="d2-m-10">
              ...
              <!-- 审合意见 -->
              <textarea v-model="form.remark" name id cols="100%" rows="4"></textarea>
            </div>
          </el-form-item>
        </div>
...
import $ from "jquery";
import {
  supervisor_notice_verify_view,
  supervisor_notice_verify
} from "@/api/sale";
...
export default {
  ...
  watch: {
    $route(to, from) {
      if (to.name == "sales-notice_verify_check_detail") {
        if (this.$route.query.id) {
          if (!this.ids || this.ids != this.$route.query.id) {
            this.fetchData(this.$route.query.id);
            this.form = {
  ...
  methods: {
    ...
    fetchData(id) {
      supervisor_notice_verify_view(id).then(res => {
        if (res && res.code === 0) {
    ...
    save() {
      ...
      var tagName = "sales-notice_verify_check_detail";
      this.loading = true
      supervisor_notice_verify(this.form, this, tagName);
    }
  }
};
</script>
