<template>
  <d2-container>
    <table border="1" bordercolor="#E5E5E5" style="width: 100%;font-size:12px;">
      ...
      <tr v-for="(items,index) in list.data" :key="index">
    ...
    <el-form :inline="true" :model="formInline" label-width="130px" class="demo-form-inline d2-m">
      <el-form-item label="待认余额" style="width:32%" v-if="$route.query.transactionpipe_id">
        <el-input v-model="formInline.pay_in_balance" size="mini" placeholder disabled></el-input>
      </el-form-item>
      <el-form-item label="第三方支付手续费" style="width:32%" v-if="$route.query.transactionpipe_id&&$route.query.currency=='RMB'">
        <el-input v-model="formInline.pay_out_balance" size="mini" placeholder disabled></el-input>
      </el-form-item>
      <el-form-item label="本次应收金额" style="width:32%">
        <el-input v-model="formInline.total_money" @input="fee()" size="mini" placeholder></el-input>
      </el-form-item>
      <el-form-item label="到账金额" style="width:32%" v-if="$route.query.pay_out_balance">
        <el-input v-model="formInline.checkin" @input="fee_checkin()" size="mini" placeholder disabled></el-input>
      </el-form-item>
      <el-form-item label="到账金额" style="width:32%" v-else>
        <el-input v-model="formInline.checkin" @input="fee_checkin()" size="mini" placeholder></el-input>
      </el-form-item>
      ...
      <!-- <el-form-item label="类别" style="width:32%" v-if="list.currentaccount.length>0">
        <el-select
          ...
          style="width: 178px;"
        >
          <el-option v-for="i in account_types" :label="i.name" :value="i.id"></el-option>
        </el-select>
      </el-form-item> -->
      ...
      <el-form-item label="收款日期" style="width:32%" :required="true">
        <el-date-picker
          ...
          size="mini"
          style="width: 178px;"
          :disabled="show_trade_date"
        ></el-date-picker>
      </el-form-item>
      <el-form-item label="收款账户" style="width:32%" :required="true">
        <el-select
          ...
          style="width: 178px;"
        >
          <el-option v-for="i in Bank" :key="i.id" :label="i.name" :value="i.id"></el-option>
        </el-select>
      </el-form-item>
        ...
        <table border="1" bordercolor="#E5E5E5" style="width: 100%;font-size:12px;" v-for="(item,index) in list.data" :key="item.id">
          ...
          <tr v-for="i in item.receipts" v-if="item.receipts.length>0" :key="i.id">
            ...
            <td style="padding-top:2px;">
              ...
              <!-- <el-button @click="cancel_list(i.id)" size="mini" type="danger" v-if="i.status==1&&i.tps.length>0">取消收款</el-button> -->
...
import {account_search,fixRecieptStatus,transactionpipe_detail} from '@/api/commission';
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      // this.payment = that.$route.query.payment?(JSON.parse(that.$route.query.payment)):'';
      this.payment = {
        pay_out_balance:that.$route.query.pay_out_balance,
        pay_in_balance:that.$route.query.pay_in_balance,
      }
      ...
      receipt_index(obj, that).then(res => {
        if (res && res.code == 0) {
          ...
          res.data.data.forEach((items, index) => {
            ...
            // Number(new Big(this.formInline.contract_all).plus(items.saler_contract_detail.total_money).minus(items.real_checkin?items.real_checkin:0).minus(items.currentaccount ? items.currentaccount : 0)).minus(items.bank_fee_recieved ? items.bank_fee_recieved : 0)
          ...
          if(this.$route.query.trade_date){
              this.show_trade_date=true
              this.formInline.checked_date=this.$route.query.trade_date
          }else{
              this.show_trade_date=false
              this.formInline.checked_date = init();
          }
          this.money_a =JSON.parse(
            JSON.stringify(Number(money))
          );
          if(this.payment.pay_out_balance||this.payment.pay_in_balance){
            this.formInline.pay_in_balance = this.payment.pay_in_balance
            if(Number(this.formInline.total_money)>Number(this.payment.pay_in_balance)&&this.$route.query.currency=='RMB'){
                this.formInline.total_money = JSON.parse(JSON.stringify(this.payment.pay_in_balance))
              }
            if(this.payment.pay_out_balance&&this.payment.pay_out_balance!=0){
              // if(this.formInline.total_money>this.payment.pay_in_balance){
              //   this.formInline.total_money = this.payment.pay_in_balance
              //   this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).div(this.formInline.pay_in_balance).times(this.payment.pay_out_balance?this.payment.pay_out_balance:0)).toFixed(4)
              //   this.formInline.checkin = Number(new Big(this.formInline.total_money).minus(this.formInline.pay_out_balance))
              // }else{
              //   this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).div(this.formInline.pay_in_balance).times(this.payment.pay_out_balance?this.payment.pay_out_balance:0)).toFixed(4)
              //   this.formInline.checkin = Number(new Big(this.formInline.total_money).minus(this.formInline.pay_out_balance))
              // }
              // this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).times(this.payment.pay_out_balance?this.payment.pay_out_balance:1)).div(this.formInline.pay_in_balance).toFixed(4)
              this.formInline.bank_fee = 0
              this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).div(this.formInline.pay_in_balance?this.formInline.pay_in_balance:1).times(this.payment.pay_out_balance?this.payment.pay_out_balance:1)).toFixed(4)
              this.formInline.checkin = Number(new Big(this.formInline.total_money).minus(this.formInline.pay_out_balance));
            }else{
              this.formInline.pay_out_balance = 0
              if(this.payment.pay_in_balance>this.formInline.contract_all){
                this.formInline.bank_fee = 0
              }else{
                this.formInline.bank_fee = Number(new Big(this.formInline.total_money).minus(this.formInline.pay_in_balance));
              }
              this.formInline.checkin = Number(new Big(this.formInline.total_money).minus(this.formInline.bank_fee));
              // if(this.formInline.total_money>this.payment.pay_in_balance){
              //   // this.formInline.checkin = Number(new Big(this.payment.pay_in_balance))
              //   // this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).minus(this.formInline.checkin)).toFixed(4)
                
              // }else{
              //   // this.formInline.checkin = Number(new Big(this.formInline.total_money))
              //   // this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).minus(this.formInline.checkin)).toFixed(4)
              
              // }
            }
          }else{
            this.formInline.pay_in_balance = 0
            this.formInline.pay_out_balance = 0
            this.formInline.checkin = Number(new Big(this.formInline.total_money))
            this.formInline.bank_fee = Number(new Big(this.formInline.total_money).minus(this.formInline.checkin))
          }
          if(this.account_id){
            this.formInline.account_id = parseInt(this.account_id)
          }
          // this.formInline.bank_fee = Number(new Big(this.formInline.total_money).minus(this.formInline.checkin));
        }
      });
      account_search().then(res => {
            ...
            this.Bank = res.data;
            if(this.account_id&&res.data.filter(i=>{return i.id==this.account_id}).length==0){
              this.$message({
                message: '没有搜索到该收款账户',
                type: "error"
              });
    ...
    fee_checkin(){
      this.formInline.bank_fee = Number(new Big(this.formInline.total_money).minus(this.formInline.checkin));
      this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).minus(this.formInline.checkin))
    },
    fee() {
      this.formInline.commission_expression =this.expression.expression;
      this.evaluate();
      this.evaluate_commission_fee();
      if(this.formInline.pay_in_balance){
        // this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).div(this.formInline.pay_in_balance).times(this.payment.pay_out_balance?this.payment.pay_out_balance:0)).toFixed(4)
        // this.formInline.checkin = Number(new Big(this.formInline.total_money).minus(this.formInline.pay_out_balance))
        // if(Number(this.formInline.checkin)>Number(this.formInline.pay_in_balance)){
        //   this.formInline.checkin = this.formInline.pay_in_balance
        // }
        if(Number(this.formInline.total_money)>Number(this.formInline.pay_in_balance)&&this.$route.query.currency=='RMB'){
          this.formInline.total_money=JSON.parse(JSON.stringify(this.formInline.pay_in_balance))
          this.$message.error('不能超过待认余额')
        }
        if(this.payment.pay_out_balance&&this.payment.pay_out_balance!=0){
          this.formInline.bank_fee = 0
          this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).div(this.formInline.pay_in_balance?this.formInline.pay_in_balance:1).times(this.payment.pay_out_balance?this.payment.pay_out_balance:1)).toFixed(4)
          // this.formInline.pay_out_balance = Number(new Big(this.formInline.pay_in_balance).times(this.payment.pay_out_balance?this.payment.pay_out_balance:1).div(this.formInline.total_money?this.formInline.total_money:1)).toFixed(4)
          // this.formInline.pay_out_balance = Number(new Big(this.formInline.total_money).div(this.formInline.pay_in_balance).times(this.payment.pay_out_balance?this.payment.pay_out_balance:0)).toFixed(4)
          this.formInline.checkin = Number(new Big(this.formInline.total_money).minus(this.formInline.pay_out_balance));
        }else{
          this.formInline.pay_out_balance = 0
    ...
    save() {
      ...
      if(this.$route.query.transactionpipe_id){
          this.formInline.transactionpipe_id = JSON.parse(this.$route.query.transactionpipe_id)
          this.formInline.first_transactionpipe_id = JSON.parse(this.$route.query.first_transactionpipe_id)[0]
      }
      this.list.data.forEach((items, index) => {
    ...
    transactionpipe_detail_(){
      if(this.$route.query.transactionpipe_id){
        var obj={
            id:JSON.parse(this.$route.query.transactionpipe_id)
        }
        transactionpipe_detail(obj).then(res=>{
          this.$route.query.pay_in_balance = res.data.pay_in_balance?res.data.pay_in_balance:''
          this.$route.query.pay_out_balance = res.data.pay_out_balance?res.data.pay_out_balance:''
          this.$route.query.account_id = res.data.account_id?res.data.account_id:''
          this.fetchData()
        })
      }else{
        this.fetchData()
    ...
    cancel() {
      ...
      that.$router.push({
                ...
                query: {
                  ...
                  transactionpipe_id:this.$route.query.transactionpipe_id?this.$route.query.transactionpipe_id:'',
      ...
      var tagName = "commission-Receipt_new";
